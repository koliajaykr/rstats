...
![](image/alicepaper.png){fig-align="center" width="15.73in"}
...
*silently Prof. Milton 😺 messaged Zach about the Reverse score*
...
-   some variables can be directly observed (height, weight or age) ... some variables you cannot measure directly (depression, anxiety, life satisfaction, relationship satisfaction)
...
-   Something that cannot be directly measured is known as **construct**.
    ...
    -   For example Relationship Assessment Scale.
...
-   **Nominal Scale**
    ...
    -   **Binary or dichotomous variable** consists of only two categories.

    -   **Coding** means when numbers are used to denote categories.

    -   The only way nominal data can be used is to consider frequencies.
:::
...
##### 😺 Prof. Milton: "The line between discrete and continuous variables can be blurred, though."

![](image/continuous_discrete.png)

## Measurement Error

-   "Ideally we want our measure to be calibrated such that values have the same meaning over time and across situations."

-   "There will often be discrepancy between the numbers we use to **represent** the thing we're measuring and the **actual value** of the thing we're measuring (i.e., the value we would get if we could measure it directly). This discrepancy is known as measurement error.

-   Self-report measures are likely to have relatively large measurement error because factors other than the one we're trying to measure will influence how people respond to questionnaires.
...
"to test the same people twice: a reliable instrument should produce similar scores at both points in time. ... However, it is complicated. ... Maybe you **new friend Milton** can help!
